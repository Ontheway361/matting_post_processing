...
void deleteboundaryUnknown(cv::Mat &trimap){
    // delete up-boundary
    for(int y = 0; y < trimap.rows; y++){
...
void globalMatting(cv::InputArray _image, cv::InputArray _trimap, cv::OutputArray _foreground, cv::OutputArray _alpha, cv::OutputArray _conf)
{
    ...
    // trans trimap to F,B,U
    for(int y = 0; y < trimap.rows; y++){
        for(int x = 0; x < trimap.cols; x++){
            if(trimap.at<uchar>(y,x) > 180){
                trimap.at<uchar>(y,x) = 255;
            }else if(trimap.at<uchar>(y,x) < 50){
    ...
    cv::imwrite("./trimap.jpg", trimap);
    // detele the boundary of trimap
    deleteboundaryUnknown(trimap);
    cv::imwrite("./deal_trimap.jpg", trimap);
