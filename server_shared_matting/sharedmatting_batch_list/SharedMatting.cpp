...
void SharedMatting::loadImage(const cv::Mat &img)
{
    pImg = img.clone();
    if (!pImg.data)
    {
...
void SharedMatting::loadTrimap(const cv::Mat &triMap)
{
    Mat trimap = triMap.clone();
   ...
   // uchar * d   = (uchar *)trimap.data;
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            m_ppTriData[i][j] = trimap.at<uchar>(i,j);
        }
    }
...
{// 找到颜色差值满足条件，离得最近的已知点，如果距离满足条件，即未知点的label是当前已知点。否则，最近的点的距离不满足条件，说明当前点被设置为unknowm。
...
double SharedMatting::energyOfPath(int i1, int j1, int i2, int j2)
{
    double ci = i2 - i1;
    double cj = j2 - j1;
    double z  = sqrt(ci * ci + cj * cj);//unknown点和样本点的 距离

    double ei = ci / (z + 0.0000001);// 单位向量的x坐标
    double ej = cj / (z + 0.0000001);//单位向量y坐标
...
double SharedMatting::colorDistance2(Scalar cs1, Scalar cs2)
{
 ...
 // cv::imwrite("trimap.jpg", cv::Mat(height, width, CV_8U, trimap.data());
...
void SharedMatting::sample(std::vector<vector<Point> > &foregroundSamples, std::vector<vector<Point> > &backgroundSamples)
{
    ...
    for(iter=unknownSet.begin();iter!=unknownSet.end();++iter) {
        vector<Point> fPts,bPts;//分别代表对于每一个未知点他的前景背景样本点

        x=iter->x;
        y=iter->y;
        angle=(x+y)*b % a; //初始化角度
        for(i=0;i<KG;++i) {// 对于每一个未知点，进行kg方向的采样
            bool f1(false),f2(false);

            z=(angle+i*a)/180.0f*3.1415926f ;// 每个射线的方向 以及转换成弧度。
            ex=sin(z); // 1 当 z 等于90 的时候
            ey=cos(z);// 0 when z is 90
            step=min(1.0f/(abs(ex)+1e-10f),
                     1.0f/(abs(ey)+1e-10f)); // when z = 90 step = 1
            ...
            for(t=0;;t+=step) {
                ...
                if(!f1 &&  IS_BACKGROUND(gray)) {
                    Point pt = Point(p, q);
                    bPts.push_back(pt);// 找到第一个背景点样本 压进向量
                    f1=true;  //flag find the background sample
                } else {
        ...
        foregroundSamples.push_back(fPts);// 存储所有未知点的前景背景样本点
...
void SharedMatting::gathering()
{
    ...
    for (int m = 0; m < size; ++m)
    {
        ...
        tuples.push_back(st);
        unknownIndex[i][j] = index; // i,j tuple 的索引
        ++index;
    }
}
// refine the results
